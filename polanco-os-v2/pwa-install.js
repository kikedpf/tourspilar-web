(function(){
  ...
  const ICON='apple-touch-icon.png?v=560';
  ...
  function refreshBranding(){
    ...
    if(manifest)manifest.href='manifest.webmanifest?v=560';
    let icon=document.querySelector('link[rel="icon"]');
    if(!icon){icon=document.createElement('link');icon.rel='icon';document.head.appendChild(icon);}
    icon.href=ICON; icon.type='image/png'; icon.setAttribute('sizes','180x180');
    let apple=document.querySelector('link[rel="apple-touch-icon"]');
    if(!apple){apple=document.createElement('link');apple.rel='apple-touch-icon';document.head.appendChild(apple);}
    apple.href=ICON; apple.setAttribute('sizes','180x180');
    const brand=document.querySelector('.brand-icon');
    if(brand){
      brand.textContent=''; brand.style.padding='0'; brand.style.overflow='hidden'; brand.style.background='#fff';
      const img=document.createElement('img');
      img.src=ICON; img.alt='P & B'; img.width=46; img.height=46;
  ...
  function buildUI(){
    ...
    bar.id='pwa-install-bar'; bar.className='pwa-install-bar'; bar.hidden=true;
    bar.innerHTML=`<div class="pwa-install-copy"><strong>Instala Polanco OS</strong><span>Copia el enlace, pégalo en Safari y añádelo a tu pantalla de inicio.</span></div><div class="pwa-install-actions"><button type="button" class="pwa-primary" data-pwa-install>Cómo instalar</button><button type="button" data-pwa-copy>Copiar enlace</button></div><button type="button" class="pwa-install-close" data-pwa-dismiss aria-label="Cerrar">×</button>`;
    const modal=document.createElement('div');
    modal.id='pwa-install-modal'; modal.className='pwa-modal'; modal.hidden=true;
    modal.innerHTML=`<div class="pwa-modal-card" role="dialog" aria-modal="true"><div class="pwa-modal-head"><div><strong>Instalar Polanco OS</strong><span id="pwa-modal-subtitle"></span></div><button type="button" class="pwa-modal-close" data-pwa-modal-close>×</button></div><div class="pwa-warning" id="pwa-install-warning"></div><div class="pwa-steps" id="pwa-install-steps"></div><button type="button" class="pwa-modal-copy" data-pwa-copy>Copiar enlace</button></div>`;
    document.body.append(bar,modal);
    bar.addEventListener('click',handleClick); modal.addEventListener('click',handleClick);
  ...
  function setSteps(){
    ...
    if(isIOS()){
      ...
      steps.innerHTML='<div class="pwa-step"><b>1</b><span>Pulsa <strong>Copiar enlace</strong>.</span></div><div class="pwa-step"><b>2</b><span>Abre <strong>Safari</strong>, pega el enlace y entra.</span></div><div class="pwa-step"><b>3</b><span>Pulsa <strong>Compartir</strong>: el cuadrado con una flecha hacia arriba.</span></div><div class="pwa-step"><b>4</b><span>Pulsa <strong>Añadir a pantalla de inicio</strong>.</span></div><div class="pwa-step"><b>5</b><span>Pulsa <strong>Añadir</strong>.</span></div>';
    }else{
      subtitle.textContent='Sigue estos pasos en Android.';
      warning.innerHTML='Abre el enlace en Chrome.';
      steps.innerHTML='<div class="pwa-step"><b>1</b><span>Abre el menú de los tres puntos.</span></div><div class="pwa-step"><b>2</b><span>Pulsa <strong>Instalar aplicación</strong>.</span></div><div class="pwa-step"><b>3</b><span>Confirma pulsando <strong>Instalar</strong>.</span></div>';
    }
  }

  function showBar(force){const bar=document.getElementById('pwa-install-bar');if(!bar||isStandalone()||(!force&&dismissed()))return;bar.hidden=false;}
  function hideBar(){const bar=document.getElementById('pwa-install-bar');if(bar)bar.hidden=true;}
  async function copyLink(button){try{await navigator.clipboard.writeText(APP_URL);if(button){const old=button.textContent;button.textContent='Enlace copiado';setTimeout(()=>button.textContent=old,1800);}}catch(_){window.prompt('Copia este enlace:',APP_URL);}}
  async function install(){if(deferredPrompt&&!isIOS()){deferredPrompt.prompt();await deferredPrompt.userChoice;deferredPrompt=null;hideBar();return;}setSteps();document.getElementById('pwa-install-modal').hidden=false;}
  function handleClick(e){if(e.target.closest('[data-pwa-install]'))install();if(e.target.closest('[data-pwa-copy]'))copyLink(e.target.closest('[data-pwa-copy]'));if(e.target.closest('[data-pwa-modal-close]'))document.getElementById('pwa-install-modal').hidden=true;if(e.target.closest('[data-pwa-dismiss]')){localStorage.setItem(DISMISS_KEY,String(Date.now()+7*86400000));hideBar();}}

  window.addEventListener('beforeinstallprompt',e=>{e.preventDefault();deferredPrompt=e;showBar(true);});
  window.addEventListener('appinstalled',()=>{deferredPrompt=null;hideBar();});
