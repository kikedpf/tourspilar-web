(async function(){
  ...
  try{
    const source=(await Promise.all(parts.map(async file=>{const response=await fetch(`${file}?v=380`,{cache:'no-store'});if(!response.ok)throw new Error(file);return response.text()}))).join('');
    (0,eval)(source);
  }catch(error){
